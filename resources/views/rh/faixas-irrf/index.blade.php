...
@section('content')
    <div class="py-12" x-data="irrfIndex({{ Js::from($faixasPorTabela) }})">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-medium text-gray-900">Tabelas de IRRF</h3>
                        @can('faixa-irrf.create')
                            <a href="{{ route('rh.faixa-irrf.create') }}"
                                ...
                                @forelse ($tabelas as $tabela)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            @else
                                                <span
                                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Inativa</span>
                                            @endif
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                            ...
                                            @can('faixa-irrf.edit')
                                                <a href="{{ route('rh.faixa-irrf.edit', $tabela) }}"
                                                    class="text-indigo-600 hover:text-indigo-900">Editar</a>
                                            @endcan
                                            @can('faixa-irrf.delete')
                                                <form action="{{ route('rh.faixa-irrf.destroy', $tabela) }}" method="POST"
                                                    class="inline"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="px-6 py-10 text-center text-sm text-gray-500">
                                            Nenhuma tabela IRRF cadastrada.
                                            <a href="{{ route('rh.faixa-irrf.create') }}"
            ...
            <div @click.outside="closeModal()"
                ...
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-800">
                        Faixas IRRF — <span x-text="selectedTabelaDesc"></span>
                    </h3>
...
@push('scripts')
    <script>
        function irrfIndex(faixasPorTabela) {
            return {
                isModalOpen: false,
                selectedFaixas: [],
                selectedTabelaDesc: '',
