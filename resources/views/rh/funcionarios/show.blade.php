...
@section('content')
    <div class="px-4 py-6 mx-auto max-w-7xl sm:px-6 lg:px-8" x-data="{ tab: 'pessoal' }">
        ...
        <div class="flex flex-col gap-4 mb-6 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <div class="flex items-center gap-2 mb-1 text-sm text-gray-500">
                    <a href="{{ route('rh.funcionarios.index') }}" class="transition-colors hover:text-indigo-600">
                        Funcionários
                    </a>
                    <span>/</span>
            ...
            {{-- Badges de status --}}
            <div class="flex flex-wrap items-center self-start gap-2">
                @if ($funcionario->ativo)
                    <span
                        class="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold text-green-700 rounded-full bg-green-50 ring-1 ring-green-600/20">
                        <span class="h-1.5 w-1.5 rounded-full bg-green-500"></span>
                        Ativo
                    </span>
                @else
                    <span
                        class="inline-flex items-center gap-1 px-3 py-1 text-xs font-semibold text-red-700 rounded-full bg-red-50 ring-1 ring-red-600/20">
                ...
                @if ($funcionario->eh_diarista)
                    <span
                        class="inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full bg-amber-50 text-amber-700 ring-1 ring-amber-600/20">
                        Diarista
                    </span>
                @endif
                ...
                @if ($funcionario->ferias_em_dobro)
                    <span
                        class="inline-flex items-center px-3 py-1 text-xs font-semibold text-red-700 rounded-full bg-red-50 ring-1 ring-red-600/20">
                        ⚠ Férias Vencidas
                    </span>
                @endif
            </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-6 sm:grid-cols-2 lg:grid-cols-4">
            {{-- Salário Bruto --}}
            <div class="p-5 bg-white rounded-lg shadow ring-1 ring-black/5">
                <p class="text-xs font-medium tracking-wide text-gray-500 uppercase">Salário Bruto</p>
                <p class="mt-1 text-xl font-bold text-gray-900">
                    R$ {{ number_format($funcionario->contrato->salario_bruto, 2, ',', '.') }}
                </p>
                <p class="mt-1 text-xs text-gray-400">Base: R$
                    {{ number_format($funcionario->contrato->salario_base, 2, ',', '.') }}
                </p>
            </div>

            {{-- Total Descontos --}}
            <div class="p-5 bg-white rounded-lg shadow ring-1 ring-black/5">
                <p class="text-xs font-medium tracking-wide text-gray-500 uppercase">Total Descontos</p>
            ...
            {{-- Salário Líquido --}}
            <div class="p-5 bg-white border-l-4 border-indigo-500 rounded-lg shadow ring-1 ring-black/5">
                <p class="text-xs font-medium tracking-wide text-gray-500 uppercase">Salário Líquido</p>
            ...
            {{-- Status Férias --}}
            <div class="p-5 bg-white rounded-lg shadow ring-1 ring-black/5">
                <p class="text-xs font-medium tracking-wide text-gray-500 uppercase">Férias</p>
                <p
                ...
                @if ($funcionario->ferias_vencidas)
                    <p class="mt-1 text-xs text-gray-400">
                        Vencimento: {{ Carbon::parse($funcionario->ferias_vencimento)->format('d/m/Y') }}
                    </p>
                @endif
            ...
            <nav class="flex gap-1 -mb-px min-w-max">
                ...
                @php
                    ...
                    $abas = [
                        ...
                        ['id' => 'dependente', 'label' => 'Dependentes', 'icon' => 'users'],
                    ];
                @endphp
                ...
                @foreach ($abas as $aba)
                    <button type="button" @click="tab = '{{ $aba['id'] }}'"
                        ...
                        @elseif($aba['icon'] === 'users')
                            <svg class="w-4 h-4" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd"
                                    d="M10 2a4 4 0 00-4 4v1H5a1 1 0 00-1 1v3.5a.5.5 0 00.5.5h1.755a2.75 2.75 0 005.49 0H15a.5.5 0 00.5-.5V8a1 1 0 00-1-1h-1V6a4 4 0 00-4-4zm-2.25 9a2.75 2.75 0 005.5 0h-5.5zM12 9h3v2h-3V9zM6.25 9h3v2h-3V9z"
                                    clip-rule="evenodd" />
                            </svg>
                        @endif
                        {{ $aba['label'] }}
        ...
        <div x-show="tab === 'pessoal'" x-transition.opacity>
            <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                    <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Dados Pessoais</h2>
                </div>
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                        ...
                        <x-rh.show-field label="CPF" :value="$funcionario->documentos->cpf ? Mascara::cpf($funcionario->documentos->cpf) : '—'" />
                        <x-rh.show-field label="RG" :value="$funcionario->documentos->rg ?? '—'" />
                        <x-rh.show-field label="Órgão Expedidor" :value="$funcionario->documentos->orgao_expedidor_rg ?? '—'" />
                        ...
                        <x-rh.show-field label="Dep. Imposto de Renda" :value="$funcionario->contrato->qtd_dependentes_ir ?? '0'" />
                        <x-rh.show-field label="Dep. Salário Família" :value="$funcionario->contrato->qtd_dependentes_salario_familia ?? '0'" />
                    </dl>
                </div>
            </div>
        </div>
        ...
        <div x-show="tab === 'contato'" x-transition.opacity>
            <div class="space-y-5">
                ...
                <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                    <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                        <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Contato</h2>
                    </div>
                    <div class="p-6">
                        <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                            <x-rh.show-field label="Telefone" :value="$funcionario->contatos->telefone ?? '—'" />
                            <x-rh.show-field label="Celular" :value="$funcionario->contatos->celular ?? '—'" />
                            <x-rh.show-field label="E-mail Corporativo" :value="$funcionario->contatos->email ?? '—'" />
                            <x-rh.show-field label="E-mail Pessoal" :value="$funcionario->contatos->email_pessoal ?? '—'" />
                        </dl>
                    </div>
                </div>
                ...
                <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                    <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                        <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Endereço</h2>
                    </div>
                    <div class="p-6">
                        <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                            <x-rh.show-field label="CEP" :value="$funcionario->endereco->cep ?? '—'" />
                            <x-rh.show-field label="Logradouro" :value="$funcionario->endereco->logradouro ?? '—'" class="lg:col-span-2" />
                            <x-rh.show-field label="Número" :value="$funcionario->endereco->numero ?? '—'" />
                            <x-rh.show-field label="Complemento" :value="$funcionario->endereco->complemento ?? '—'" />
                            <x-rh.show-field label="Bairro" :value="$funcionario->endereco->bairro ?? '—'" />
                            <x-rh.show-field label="Cidade" :value="$funcionario->endereco->cidade ?? '—'" />
                            <x-rh.show-field label="Estado" :value="$funcionario->endereco->estado ?? '—'" />
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div x-show="tab === 'contrato'" x-transition.opacity>
            <div class="space-y-5">
                ...
                <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                    <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                        <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Vínculo Empregatício</h2>
                    </div>
                    <div class="p-6">
                        <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                            <x-rh.show-field label="Departamento" :value="$funcionario->departamento?->nome ?? '—'" />
                            <x-rh.show-field label="Cargo" :value="$funcionario->cargo?->titulo ?? '—'" />
                            <x-rh.show-field label="Local de Trabalho" :value="$funcionario->contrato->local_trabalho ?? '—'" />
                            <x-rh.show-field label="Tipo de Contrato" :value="$funcionario->contrato->tipo_contrato ?? '—'" />
                            <x-rh.show-field label="Tipo de Contratação" :value="$funcionario->contrato->tipo_contratacao ?? '—'" />
                            <x-rh.show-field label="Data de Admissão" :value="$funcionario->contrato->data_admissao
                                ? $funcionario->contrato->data_admissao->format('d/m/Y')
                                : '—'" />
                            <x-rh.show-field label="Data de Demissão" :value="$funcionario->contrato->data_demissao
                                ? $funcionario->contrato->data_demissao->format('d/m/Y')
                                : '—'" />
                            <x-rh.show-field label="Tempo de Empresa" :value="$funcionario->tempo_empresa" />

                            <div>
                                <dt class="text-xs font-medium tracking-wide text-gray-500 uppercase">Situação</dt>
                                <dd class="mt-1">
                                    @if ($funcionario->ativo)
                                        <span
                            ...
                            <div>
                                <dt class="text-xs font-medium tracking-wide text-gray-500 uppercase">Diarista</dt>
                                <dd class="mt-1">
                                    @if ($funcionario->contrato->eh_diarista)
                                        <span
                            ...
                            @if ($funcionario->contrato->eh_diarista)
                                <x-rh.show-field label="Valor da Diária" :value="'R$ ' .
                                    number_format($funcionario->contrato->valor_diaria ?? 0, 2, ',', '.')" />
                            @endif
                        </dl>
                    </div>
                </div>
                ...
                <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                    <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                        <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Jornada de Trabalho</h2>
                    </div>
                    <div class="p-6">
                        <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-5">
                            <x-rh.show-field label="Carga Horária Semanal" :value="$funcionario->contrato->carga_horaria_semanal
                                ? $funcionario->contrato->carga_horaria_semanal . 'h'
                                : '—'" />

                            <x-rh.show-field label="Entrada" :value="$funcionario->contrato->horario_entrada
                                ? Carbon::parse($funcionario->contrato->horario_entrada)->format('H:i')
                                : '—'" />
                            <x-rh.show-field label="Saída" :value="$funcionario->contrato->horario_saida
                                ? Carbon::parse($funcionario->contrato->horario_saida)->format('H:i')
                                : '—'" />
                            <x-rh.show-field label="Almoço Início" :value="$funcionario->contrato->horario_almoco_inicio
                                ? Carbon::parse($funcionario->contrato->horario_almoco_inicio)->format('H:i')
                                : '—'" />
                            <x-rh.show-field label="Almoço Fim" :value="$funcionario->contrato->horario_almoco_fim
                                ? Carbon::parse($funcionario->contrato->horario_almoco_fim)->format('H:i')
                                : '—'" />

                            <div>
                                <dt class="text-xs font-medium tracking-wide text-gray-500 uppercase">6º Dia Útil no Mês
                                </dt>
                                <dd class="mt-1">
                ...
                @if ($funcionario->observacoes)
                    <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                        <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                            <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Observações</h2>
                        </div>
                        <div class="p-6">
        ...
        <div x-show="tab === 'documentos'" x-transition.opacity>
            <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                    <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Documentos</h2>
                </div>
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                        <x-rh.show-field label="CTPS – Número" :value="$funcionario->documentos->ctps_numero ?? '—'" />
                        <x-rh.show-field label="CTPS – Série" :value="$funcionario->documentos->ctps_serie ?? '—'" />
                        <x-rh.show-field label="CTPS – UF" :value="$funcionario->documentos->ctps_uf ?? '—'" />
                        <x-rh.show-field label="CTPS – Data de Emissão" :value="$funcionario->documentos->ctps_data_emissao
                            ? $funcionario->documentos->ctps_data_emissao->format('d/m/Y')
                            : '—'" />
                        <x-rh.show-field label="PIS/PASEP" :value="$funcionario->documentos->pis_pasep ?? '—'" />
                        <x-rh.show-field label="Título de Eleitor" :value="$funcionario->documentos->titulo_eleitor ?? '—'" />
                        <x-rh.show-field label="Zona Eleitoral" :value="$funcionario->documentos->zona_eleitoral ?? '—'" />
                        <x-rh.show-field label="Seção Eleitoral" :value="$funcionario->documentos->secao_eleitoral ?? '—'" />
                        <x-rh.show-field label="Certificado de Reservista" :value="$funcionario->documentos->certificado_reservista ?? '—'" />
                    </dl>
                </div>
            </div>
        </div>
                                                                                                                            ...
                                                                                                                            <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                                                                                                                                ...
                                                                                                                                <div class="p-6">
                                                                                                                                    <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                                                                                                                                        ...
                                                                                                                                        <x-rh.show-field label="Faltas (Qtd.)" :value="$funcionario->faltas ??
                                                                                                                                            '0'" />
                                                                                                                                        <x-rh.show-field label="DSR Faltas" :value="'R$ ' .
                                                                                                                                            number_format(
                                                                                                                                                $funcionario->dsr_faltas ??
                                                                                                                                                    0,
                                                                                                                                                2,
                                                                                                                                                ',',
                                                                                                                                                '.',
        ...
        <div x-show="tab === 'beneficios'" x-transition.opacity>
            <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                    <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Benefícios</h2>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        {{-- Vale Transporte --}}
                        <div
                            class="rounded-lg border p-4 {{ $funcionario->beneficios->vale_transporte ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50' }}">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-semibold text-gray-700">Vale Transporte</span>
                                @if ($funcionario->beneficios->vale_transporte)
                                    <span
                            ...
                            <p
                                class="text-xl font-bold {{ $funcionario->beneficios->vale_transporte ? 'text-green-700' : 'text-gray-400' }}">
                                R$ {{ number_format($funcionario->beneficios->valor_vale_transporte ?? 0, 2, ',', '.') }}
                            </p>
                        </div>

                        {{-- Vale Alimentação --}}
                        <div
                            class="rounded-lg border p-4 {{ $funcionario->beneficios->vale_alimentacao ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50' }}">
                            <div class="flex items-center justify-between mb-2">
                                <span class="text-sm font-semibold text-gray-700">Vale Alimentação</span>
                                @if ($funcionario->beneficios->vale_alimentacao)
                                    <span
                                        class="text-xs font-medium text-green-700 bg-green-100 rounded-full px-2 py-0.5">Ativo</span>
                                @else
                                    <span
                            ...
                            <p
                                class="text-xl font-bold {{ $funcionario->beneficios->vale_alimentacao ? 'text-green-700' : 'text-gray-400' }}">
                                R$ {{ number_format($funcionario->beneficios->valor_vale_alimentacao ?? 0, 2, ',', '.') }}
                            </p>
                        </div>

                        {{-- Plano de Saúde --}}
                        <div
                            class="rounded-lg border p-4 {{ $funcionario->beneficios->plano_saude ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-gray-50' }}">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-semibold text-gray-700">Plano de Saúde</span>
                                @if ($funcionario->beneficios->plano_saude)
                                    <span
                        ...
                        {{-- Plano Odontológico --}}
                        <div
                            class="rounded-lg border p-4 {{ $funcionario->beneficios->plano_odontologico ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-gray-50' }}">
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-semibold text-gray-700">Plano Odontológico</span>
                                @if ($funcionario->beneficios->plano_odontologico)
                                    <span
                                        class="text-xs font-medium text-blue-700 bg-blue-100 rounded-full px-2 py-0.5">Ativo</span>
                                @else
                                    <span
        ...
        <div x-show="tab === 'ferias'" x-transition.opacity>
            <div class="space-y-5">
                ...
                <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                    <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                        <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Status das Férias</h2>
                    </div>
                    <div class="p-6">
                        <div
                        ...
                        <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                            ...
                            <x-rh.show-field label="Vencimento das Férias" :value="$funcionario->ferias_vencimento
                                ? Carbon::parse($funcionario->ferias_vencimento)->format('d/m/Y')
                                : '—'" />
                            <x-rh.show-field label="Dias para Vencimento" :value="$funcionario->temDireitoFerias()
                                ...
                                : '—'" />
                            <div>
                                <dt class="text-xs font-medium tracking-wide text-gray-500 uppercase">Férias em Dobro</dt>
                                <dd class="mt-1">
                                    @if ($funcionario->ferias_em_dobro)
                                        <span
                ...
                @if ($funcionario->periodoFerias && $funcionario->periodoFerias->count() > 0)
                    <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                        <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                            <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Histórico de Férias
                                Gozadas</h2>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 py-3 text-xs font-medium text-left text-gray-500 uppercase">Início
                                        </th>
                                        <th class="px-4 py-3 text-xs font-medium text-left text-gray-500 uppercase">Fim
                                        </th>
                                        <th class="px-4 py-3 text-xs font-medium text-left text-gray-500 uppercase">Dias
                                        </th>
                                        <th class="px-4 py-3 text-xs font-medium text-left text-gray-500 uppercase">Status
                                        </th>
                                    </tr>
                                </thead>
                                    ...
                                    @foreach ($funcionario->periodoFerias->sortByDesc('created_at') as $periodo)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-4 py-3 text-sm text-gray-900">
                                                {{ $periodo->data_inicio ? Carbon::parse($periodo->data_inicio)->format('d/m/Y') : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-sm text-gray-900">
                                                {{ $periodo->data_fim ? Carbon::parse($periodo->data_fim)->format('d/m/Y') : '—' }}
                                            </td>
                                            <td class="px-4 py-3 text-sm text-gray-900">
                                                {{ $periodo->dias ?? '—' }}
                                            </td>
        ...
        <div x-show="tab === 'bancario'" x-transition.opacity>
            <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                    <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Dados Bancários</h2>
                </div>
                <div class="p-6">
                    <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                        <x-rh.show-field label="Banco (Código)" :value="$funcionario->dadosBancarios->banco_codigo ?? '—'" />
                        <x-rh.show-field label="Banco (Nome)" :value="$funcionario->dadosBancarios->banco_nome ?? '—'" class="lg:col-span-2" />
                        <x-rh.show-field label="Agência" :value="$funcionario->dadosBancarios->agencia ?? '—'" />
                        <x-rh.show-field label="Conta" :value="$funcionario->dadosBancarios->conta ?? '—'" />
                        <x-rh.show-field label="Tipo de Conta" :value="$funcionario->dadosBancarios->tipo_conta ?? '—'" />
                    </dl>
                </div>
            </div>
        </div>
        {{-- ======= ABA: DEPENDENTES ========= --}}
        <div x-show="tab === 'dependente'" x-transition.opacity>
            <div class="overflow-hidden bg-white rounded-lg shadow ring-1 ring-black/5">
                <div class="px-6 py-3 border-b border-gray-100 bg-gray-50">
                    <h2 class="text-sm font-semibold tracking-wide text-gray-700 uppercase">Dependentes</h2>
                </div>
                <div class="p-6">
                    <!-- Conteúdo dos dependentes -->
                    <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-x-6 gap-y-5">

                        @foreach ($funcionario->dependentes as $dependente)
                            <x-rh.show-field label="Nome" :value="$dependente->nome_completo ?? '—'" />
                            <x-rh.show-field label="Parentesco" :value="$dependente->parentesco ?? '—'" />
                            <x-rh.show-field label="Data de Nascimento" :value="$dependente->data_nascimento
                                ? Carbon::parse($dependente->data_nascimento)->format('d/m/Y')
                                : '—'" />
                            <x-rh.show-field label="Dependente para Imposto de Renda" :value="$dependente->dependente_ir ? 'Sim' : 'Não'" />
                            <x-rh.show-field label="Dependente para Salário Família" :value="$dependente->dependente_sf ? 'Sim' : 'Não'" />
                        @endforeach
                    </dl>
                    {{-- inserir uma linha para divisão --}}
                    <div class="border-t border-gray-200 mt-6"></div>
                    <div class="mt-4">
                        <p class="text-sm text-gray-500">Total de Dependentes:
                            <strong>{{ $funcionario->dependentes->count() }}</strong>
                        </p>
                        <p class="text-sm text-gray-500">Total de Dependentes para Imposto de Renda:
                            <strong>{{ $funcionario->dependentes->where('dependente_ir', true)->count() }}</strong>
                        </p>
                        <p class="text-sm text-gray-500">Total de Dependentes para Salário Família:
                            <strong>{{ $funcionario->dependentes->where('dependente_sf', true)->count() }}</strong>
                        </p>
                    </div>
                </div>
        ...
        <div class="px-6 py-4 mt-6 bg-white rounded-lg shadow ring-1 ring-black/5">
