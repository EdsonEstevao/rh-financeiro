    ...
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                📊 Tabela INSS → Editar {{ $faixaInss->numero_faixa }}ª Faixa
            </h2>
        </div>
    </div>
    <div class="py-6">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <form action="{{ route('rh.faixa-inss.update', $faixaInss) }}" method="POST" class="p-6 space-y-5">
                    @csrf @method('PUT')

                    <div>
                        <x-input-label for="numero_faixa" value="Número da Faixa" />
                        <x-text-input id="numero_faixa" name="numero_faixa" type="number" min="1"
                            class="mt-1 block w-full" :value="old('numero_faixa', $faixaInss->numero_faixa)" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="limite_inferior" name="limite_inferior" type="number" step="0.01"
                                min="0" class="mt-1 block w-full" :value="old('limite_inferior', $faixaInss->limite_inferior)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="limite_superior" name="limite_superior" type="number" step="0.01"
                                min="0" class="mt-1 block w-full" :value="old('limite_superior', $faixaInss->limite_superior)" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="aliquota" name="aliquota" type="number" step="0.01" min="0"
                                max="100" class="mt-1 block w-full" :value="old('aliquota', $faixaInss->aliquota)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="parcela_deduzir" name="parcela_deduzir" type="number" step="0.01"
                                min="0" class="mt-1 block w-full" :value="old('parcela_deduzir', $faixaInss->parcela_deduzir)" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="vigencia_inicio" name="vigencia_inicio" type="date"
                                class="mt-1 block w-full" :value="old('vigencia_inicio', $faixaInss->vigencia_inicio->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="vigencia_fim" name="vigencia_fim" type="date" class="mt-1 block w-full"
                                :value="old('vigencia_fim', $faixaInss->vigencia_fim?->format('Y-m-d'))" />
                    ...
                    <div class="flex items-center gap-3">
                        <input id="ativa" name="ativa" type="checkbox" value="1"
                            {{ old('ativa', $faixaInss->ativa) ? 'checked' : '' }}
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t dark:border-gray-700">
                        ...
                        <x-primary-button>💾 Atualizar Faixa</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
